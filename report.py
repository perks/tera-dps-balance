...
<p class="lead">Median / 90th percentile / best recorded DPS per class, 5-man kills only. <span class="n">n = samples · pl = distinct named players</span></p>
<div class="panel">
...
const lenses=[
 {id:'raw',label:'Raw (all kills)',rows:Object.entries(D.relIndex),key:'avg',nkey:'n / players',note:`Average of per-kill index. Player-best median (each player counted once): ${Object.entries(D.relIndex).sort((a,b)=>b[1].playerBestMed-a[1].playerBestMed).map(([c,r])=>c+' '+f2(r.playerBestMed)).join(' · ')}.`},
 {id:'kt',label:'Kill-time matched',rows:Object.entries(D.killTimeIndex).map(([c,r])=>[c,{avg:r.rel,n:r.n,players:D.relIndex[c].players}]),key:'avg',nkey:'n / players',note:'Class average ÷ median of the same duration quartile on the same boss, weighted by sample count across all 15 bosses.'},
 {id:'gear',label:'Same gear',rows:Object.entries(D.gear.stdBucket.classes),key:'avg',nkey:'n / players',note:`Only players wearing ${D.gear.stdBucket.desc} (${D.gear.stdBucket.n} samples).`},
 {id:'top',label:'Top 10% of players',rows:Object.entries(D.relIndex).map(([c,r])=>[c,{avg:r.top10,n:r.n,players:r.players}]),key:'avg',nkey:'n / players',note:'Average index of the best 10% of samples per class — the ceiling, not the typical player.'}
];
...
function areaTable(a){const rows=Object.entries(D.byArea[a]).sort((x,y)=>y[1].med-x[1].med);
 document.getElementById('areaTable').innerHTML=`<table><thead><tr><th>class</th><th>n</th><th>pl</th><th>min</th><th>p25</th><th>median</th><th>avg</th><th>p75</th><th>p90</th><th>max</th><th>index</th></tr></thead><tbody>`+
